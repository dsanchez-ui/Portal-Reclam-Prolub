...
function doGet(e) {
  ...
  try {
    ...
    for (var i = 1; i < rows.length; i++) {
      ...
      var mitigationJson = (row.length > 20) ? row[20] : "{}"; 
      var mitigationData = parseJSONSafe(mitigationJson); 

      var filesJson = (row.length > 21) ? row[21] : "[]";
      ...
      var claim = {
        ...
        driveFolderUrl: row[13], // URL de la carpeta
        internalCloseDate: formatDate(row[16]), // Column 17 (Index 16) - Reading Close Date
        assignedTo: row[17],
        ishikawaList: parseJSONSafe(row[18]), 
        tasks: parseJSONSafe(row[19]),
        
        immediateSolution: mitigationData.description || "", 
        immediateSolutionResponsible: mitigationData.responsible || "", 
        immediateSolutionStatus: mitigationData.status || "Pending", 
        immediateSolutionDate: mitigationData.date || "",
        immediateSolutionExecutionNotes: mitigationData.notes || "",
        immediateSolutionExecutionEvidence: mitigationData.evidence || [],
...
function doPost(e) {
  ...
  try {
    ...
    // --- ACCIÓN: CIERRE SIMPLE (SOLICITUD ESPECÍFICA) ---
    if (action === 'close_case_simple') {
       var values = sheet.getDataRange().getValues();
       var rowIndex = -1;
       // Buscar ID en Columna A (Indice 0)
       for (var i = 1; i < values.length; i++) {
         if (values[i][0] == data.id) { rowIndex = i + 1; break; }
       }

       if (rowIndex > 0) {
         // 1. Actualizar Columna C (Status) a 'Cerrado'
         sheet.getRange(rowIndex, 3).setValue('Cerrado'); // Col 3 = C
         
         // 2. Actualizar Columna Q (Internal Close Date) a la fecha enviada
         // Si no se envia fecha, usar la actual del servidor
         var closeDate = data.date || Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "dd/MM/yyyy");
         sheet.getRange(rowIndex, 17).setValue(closeDate); // Col 17 = Q

         return ContentService.createTextOutput(JSON.stringify({result: 'success', action: 'closed_simple'})).setMimeType(ContentService.MimeType.JSON);
       } else {
    ...
    if (action === 'delete') {
       ...
       if (rowIndex > 0) {
         ...
         return ContentService.createTextOutput(JSON.stringify({result: 'success', action: 'deleted_soft'})).setMimeType(ContentService.MimeType.JSON);
       } else {
         return ContentService.createTextOutput(JSON.stringify({result: 'error', message: 'ID not found'})).setMimeType(ContentService.MimeType.JSON);
       }
    }

    // --- ACCIÓN: GUARDAR PDF EN DRIVE ---
    if (action === 'save_pdf') {
       var rootFolder = DriveApp.getFolderById("1PJBsgGwyR1BLG8X6wutmwvXs0ItYoLl2"); // ID CARPETA RAIZ
       var targetFolder;
       
       // Buscar carpeta existente del caso
    ...
    if (uploadedFileInfos.length > 0) {
        ...
        for (var u = 0; u < uploadedFileInfos.length; u++) {
            if (uploadedFileInfos[u].name.indexOf("EVIDENCIA_MITIGACION") !== -1) {
                if (!data.immediateSolutionExecutionEvidence) data.immediateSolutionExecutionEvidence = [];
                data.immediateSolutionExecutionEvidence.push(uploadedFileInfos[u]);
            }
        }
    }

    var readableIshikawa = "";
    if (data.ishikawaList && data.ishikawaList.length > 0) {
      readableIshikawa = data.ishikawaList.map(function(item) { return "• [" + item.category + "]: " + item.observation; }).join("\n");
    }
    var readableTasks = "";
    if (data.tasks && data.tasks.length > 0) {
    ...
    // Determine Closing Date: Prefer payload data, fallback to auto-calc if status is Closed
    ...
    if (action === 'create') {
      var mitigationData = {
          description: data.immediateSolution || "",
          responsible: data.immediateSolutionResponsible || "",
          status: data.immediateSolutionStatus || "Pending",
          notes: "",
          evidence: []
      };
      ...
      var rowData = [
        ...
        productRefString, 
        data.batch, data.incidentType, data.description, data.correctionType, 
        targetFolder ? targetFolder.getUrl() : "", 
        readableIshikawa, 
        readableTasks, 
        closingDate || "", 
        data.assignedTo || '',
        JSON.stringify(data.ishikawaList || []), 
        JSON.stringify(data.tasks || []), 
        JSON.stringify(mitigationData),
        JSON.stringify(initialEvidenceInfos)
      ];
      ...
      if (rowIndex > 0) {
        ...
        var mitigationObj = {
             ...
             responsible: data.immediateSolutionResponsible || "",
             status: data.immediateSolutionStatus || "Pending",
             date: data.immediateSolutionDate || "",
             notes: data.immediateSolutionExecutionNotes || "",
             evidence: data.immediateSolutionExecutionEvidence || [] 
        };
        sheet.getRange(rowIndex, 21).setValue(JSON.stringify(mitigationObj));
